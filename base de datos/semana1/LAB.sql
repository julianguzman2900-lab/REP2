...
SELECT * 
FROM productos 
WHERE NOT categoria = 'Electrónica' AND stock > 0 
ORDER BY categoria;

-- LIMIT
-- 1
SELECT * 
FROM productos LIMIT 2;
-- 2
SELECT * 
FROM productos 
ORDER BY precio DESC 
...
SELECT * 
FROM productos 
WHERE proveedor_id IS NOT NULL AND stock IS NULL;
...
SELECT * 
FROM productos 
WHERE stock BETWEEN 5 AND 20 AND categoria = 'Mobiliario';
...
-- 3
SELECT nombre, (precio * 0.13) AS Impuesto_Ventas 
FROM productos;
...
FROM productos;
-- 2
SELECT CONCAT('El producto ', nombre, ' cuesta ', precio) 
FROM productos;
-- 3
SELECT CONCAT(id, categoria) AS codigo 
FROM productos;
...
GROUP BY categoria;
-- 3
SELECT proveedor_id, MAX(precio), MIN(precio) 
FROM productos 
...
-- 3
SELECT proveedor_id, SUM(stock) AS total_stock, SUM(precio * stock) AS valor_total 
FROM productos 
GROUP BY proveedor_id 
HAVING SUM(stock) > 1 AND valor_total > 500;